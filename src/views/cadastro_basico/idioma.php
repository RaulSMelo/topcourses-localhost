...
if (isset($_POST["btn-salvar"])) {
    ...
    $ret = $cadBasicoCTRL->inserirCadastroBasico(IDIOMA);
    
}else if (isset($_POST["btn-alterar"])) {

    $ret = $cadBasicoCTRL->alterarCadastroBasico(IDIOMA);
    ...
    <div class="container padding-card-pattern">
        <div class="card my-3 main-card">
            <div class="card-header text-left mt-0 header bg-dark">
                ...
                <h5><em>Aqui você pode cadastrar os idiomas</em></h5>
            </div>
                    ...
                    <div class="col-12">
                        ...
                        <form action="idioma.php" method="POST">
                            <div class="form-group">
                                <label class="input-title" for="idioma">Nome do idioma</label>
                                <input class="form-control form-control-sm campo-obrigatorio" type="text" id="idioma" name="nome-cadastro" placeholder="Idioma" autocomplete="off">
                                <small id="msg-erro-idioma" class="text-danger invisible">Preencha o campo idioma</small>
                            </div>
                                        ...
                                        <?php for ($i = 0; $i < count($idiomas); $i++) : ?>
                                            ...
                                            <tr>
                                                <td class="line-nowrap">
                                                    <?= $idiomas[$i]["nome_cadastro"] ?>
                                                </td>
                                                <td class="line-nowrap">
                                                    <a onclick="modalAlterarCadastroBasico('<?= $idiomas[$i]["id_cadastro_basico"] ?>', '<?= $idiomas[$i]["nome_cadastro"] ?>')" class="btn btn-warning btn-sm" data-toggle="modal" data-target="#modal-alterar">
                                                        <i class="fas fa-pencil-alt"></i>
                                                        Alterar
                                                    </a>
                                                    <a onclick="modalExcluirCadastroBasico('<?= $idiomas[$i]["id_cadastro_basico"] ?>', '<?= $idiomas[$i]["nome_cadastro"] ?>')" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#modal-excluir">
